...
const validateStep2 = function() {
    if(!$("input[name='chkCity']").is(":checked")) {
        toastr["error"]("Debes seleccionar la(s) ciudad(es) donde operas.");
        return false;
    }
    if($("#cbMainCity").find("option:selected").val() == "") {
        toastr["error"]("Debes seleccionar la ciudad de operación principal.");
        $("#cbMainCity").focus();
        return false;
    }
    if($("#cbEmployee").find("option:selected").val() == "") {
        toastr["error"]("Debes seleccionar la cantidad de empleados.");
        $("#cbEmployee").focus();
        return false;
    }
    if($("#cbContract").find("option:selected").val() == "") {
        toastr["error"]("Debes seleccionar el tipo de contrato de tus empleados.");
        $("#cbContract").focus();
        return false;
    }
    if($("#cbClients").find("option:selected").val() == "") {
        toastr["error"]("Debes seleccionar si deseas o no vincular tus clientes.");
        $("#cbClients").focus();
        return false;
    }
    return true;
