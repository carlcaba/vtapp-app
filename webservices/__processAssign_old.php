	...
	if($_SERVER['REQUEST_METHOD'] != 'PUT') {
		if(!isset($_POST['user'])) {
			if(!isset($_GET['user'])) {
				...
				$step = $_GET['step'];
				$id = $_GET['id'];
				$cancel = (isset($_GET['cancel']) ? filter_var($_GET['cancel'],FILTER_VALIDATE_BOOLEAN) : $cancel);
			}
		}
		else {
			$user = $_POST['user'];
			...
			$step = $_POST['step'];
			$id = $_POST['id'];
			$cancel = (isset($_POST['cancel']) ? filter_var($_POST['cancel'],FILTER_VALIDATE_BOOLEAN) : $cancel);
		}
	} 
	else {
		//Captura las variables
