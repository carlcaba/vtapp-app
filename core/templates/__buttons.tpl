...
?>
									<button type="button" data-toggle="tooltip" data-placement="top" title="<?= $_SESSION["LOAD_TEMPLATE"] ?>" id="btnLoad" name="btnLoad" class="btn btn-success pull-right" onclick="upload();" <?= $disabled ?>>
									...
									<form id="frmDownloadTemplate" name="frmDownloadTemplate" method="get" action="templates/<?= $template ?>" >
										<button type="button" data-toggle="tooltip" data-placement="top" title="<?= $_SESSION["DOWNLOAD_TEMPLATE"] ?>" id="btnTemplate" name="btnTemplate" class="btn btn-block bg-light pull-right" onclick="document.getElementById('frmDownloadTemplate').submit();" <?= $disabled ?>>
