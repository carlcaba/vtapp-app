...
class order_state extends table {
	var $resources;
	var $view;
	
	//Constructor
	function __constructor($order_state = "") {
	...
	function order_state($orderstate = '') {
		...
		$this->RESOURCE_NAME = $orderstate;
		//Especifica los valores unicos
		$this->_addUniqueColumn("ID");
		//Relaciones con otras clases
		$this->resources = new resources();
		$this->view = "VIE_ORDER_STATE_SUMMARY";		
	}
	...
	//Funcion que busca el nombre del order_state
	...
	//Funcion que activa o habilita a una order_state
	...
	//Funcion que despliega los valores en un order_state
	...
	function showSummary($aColumnsBD,$sWhere,$sOrder,$sLimit) {
		$fields = ["ORDER_STATE_ID", "STATE_NAME", "BADGE", "REGISTERED_ON", "REGISTERED_BY", "IS_BLOCKED", "LANGUAGE_ID"];
		//Verifica el where
		if($sWhere != "")
	...
	function showForm($action, $tabs = 5) {
		...
		if($action == "new") {
			...
			$link = "core/actions/_save/__newState.php";
		}
		else if($action == "edit") {
			$readonly = array("readonly=\"readonly\"", "", "", "disabled", "disabled", "");
			$action = $_SESSION["EDIT"];
			$link = "core/actions/_save/__editState.php";
		}
		else {
			...
			$link = "core/actions/_save/__deleteState.php";
		}
		...
		$showvalue = true;
		//variable a retornar
		$return = "$stabs<form id=\"frmState\" name=\"frmState\" role=\"form\">\n";
		//Muestra la GUI
		...
		$return .= $this->showField("BADGE", "$stabs\t", "", "", $showvalue, "", false, "9,9,12", $readonly[$cont++]);
