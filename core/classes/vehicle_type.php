...
class vehicle_type extends table {
	...
	//Constructor
	function __constructor($vehicle_type = "") {
	...
	function vehicle_type($vehicle_type = '') {
		...
		$this->RESOURCE_NAME = $vehicle_type;
	...
	//Funcion para verificar si puede seleccionar un vehiculo
	function isForSelect() {
		return $this->RESOURCE_NAME == "VEHICLE_TYPE_6";
	}

	//Funcion para verificar el id para seleccionar
	function IDForSelect() {
	    //Arma la sentencia SQL
        $this->sql = "SELECT ID FROM $this->table WHERE RESOURCE_NAME = 'VEHICLE_TYPE_6'";
        //Variable a retornar
        $result = 0;
        //Obtiene los resultados
        $row = $this->__getData();
