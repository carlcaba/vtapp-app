...
class zone extends table {
	...
	//Funcion para obtener la zona padre
	function getParentZoneName() {
		$result = $this->PARENT_ZONE;
		if($this->PARENT_ZONE != "") {
			//Arma la sentencia SQL
			$this->sql = "SELECT ZONE_NAME FROM $this->table WHERE ID = $this->PARENT_ZONE";
			//Obtiene los resultados
			$row = $this->__getData();
			//Registro no existe
			if($row) {
