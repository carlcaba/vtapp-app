...
class report2 extends table {
	...
	function report2() {
		...
		parent::__constructor("VIE_REPORT2_SUMMARY");
		...
		$return = "
									<form id=\"frmFilter\" name=\"frmFilter\" role=\"form\">
										<div class=\"form-group\">
											<label>" . $_SESSION["WAREHOUSE"] . "<span class=\"required\">*</span></label>
											<select class=\"form-control select2-id\" id=\"ID_AREA\" name=\"ID_AREA\" style=\"width: 100%;\">
												<option value=\"*\" selected>" . $_SESSION["ALL_WAREHOUSES"] . "</option>";
		$return .= $this->movement->product->area->showWarehouseList(8,"",0,false);
		$return .= "
											</select>
										</div>
										<div class=\"form-group\">
										...
										<input type=\"hidden\" name=\"hfReport\" id=\"hfReport\" value=\"report2\" />
										<input type=\"hidden\" name=\"hfClass\" id=\"hfClass\" value=\"report2\" />
									</form>\n";
		return $return;
	}
	...
	function showHeaders($lang = 0, $array = false) {
		...
		$this->sql = "SELECT RESOURCE_TEXT, RESOURCE_NAME FROM " . $this->resources->table . " WHERE RESOURCE_NAME LIKE 'REPORT2%' AND LANGUAGE_ID = $lang ".
	...
	function filterData($reporte = "report2", $lang = 0) {
		//Verifica el lenguaje
		if($lang == 0) {
			//Lenguaje establecido
			$lang = $_SESSION["LANGUAGE"];
		...
		if($reporte == "report2") {
			...
			if($this->ID_AREA != "" && $this->ID_AREA != "*")
				$this->sql .= "AND ID_AREA = " . $this->_checkDataType("ID_AREA") . " ";
			if($this->MOVE_DATE != "") {
	...
	function showPrint($company, $lang = 0) {
		...
		if($this->MOVE_DATE != "") {
			$dates = explode(" / ",$this->MOVE_DATE);
			$subtitle2 = $_SESSION["REPORT2_SUBTITLE_2"];
			if(count($dates) > 1) {
		...
		$return = "
			...
			<div class=\"row\">
				<div class=\"col-12\">
					<h2 class=\"page-header text-center\">" . sprintf($_SESSION["REPORT2_TITLE"],$company) . "</h2>
					<h3 class=\"page-header text-center\">" . $_SESSION["REPORT2_SUBTITLE"] . "</h3>"
		...
		$return .= "
			...
			<div class=\"row\">
				<div class=\"col-12\">
					<h4 class=\"page-header text-center\">" . $_SESSION["REPORT2_SUBTITLE_3"] . "</h4>
				</div>
				<!-- /.col -->
			</div>";
	...
	function exportToExcel(&$objPHPExcel, $filename, $company, $lang = 0) {
		...
		$objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('B1', $_SESSION["REPORT2_TITLE"])
                            ->setCellValue('B2', $_SESSION["REPORT2_SUBTITLE"])
                            ->setCellValue('B3', $subtitle2)
                            ->setCellValue('B4', $generated)
                            ->setCellValue('B5', $_SESSION["REPORT2_SUBTITLE_3"]);
