	...
	function showTableDataJSON($type) {
		...
		foreach($this->__getAllData() as $row) {
			...
			$data = array("id" => $row[0],
						...
						"zone_id" => $row[9],
						"parent_zone" => $row[10],
						"button" => "<button type=\"button\" class=\"btn btn-warning btn-sm\" id=\"btnSelectAddress\" name=\"btnSelectAddress\" data-address=\"$address\" data-latitude=\"$row[7]\" data-longitude=\"$row[8]\">" . $_SESSION["SELECT"] . "</button>"
					);
			array_push($return,$data);
		}
		//Retorna
