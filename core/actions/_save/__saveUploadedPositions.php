<?
	...
	if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
		...
		$reso = new resources();
		$headers = explode(",",$reso->getResourceByName("POSITIONS_HEADER"));
		...
		if(substr($usua->access->PREFIX,0,2) == "CL") {
			...
			$position->setClient($idcli);
			if($position->nerror > 0) {
		...
		$responseHeader = explode(",",$_SESSION["POSITIONS_RESPONSE_HEADER"]);
		// Add some data
				...
				if(strpos($key,"hfFile") !== false) {
					...
					if(file_exists($fname)) {
						...
						for($i = 2; $i <= $highestRow; $i++) {
							$counter++;
							//Asigna la informacion
							$position->POSITION_NAME = $objWorksheet->getCellByColumnAndRow(0,$i)->getValue();
							$position->TITLE = $objWorksheet->getCellByColumnAndRow(1,$i)->getValue();
							$position->area->AREA_NAME = $objWorksheet->getCellByColumnAndRow(2,$i)->getValue();
							$position->area->getInformationByName($position->area->AREA_NAME);
							//Si no existe el area
							if($position->area->nerror > 0) {
								$errors++;
								$error = "File: $value - " . $position->area->AREA_NAME . " : cargo " . $position->POSITION_NAME . " Area error: " . $position->area->error;
								array_push($arrErr,$error);
								//Adiciona al response
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(0,$rowResponse,$position->POSITION_NAME);
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(1,$rowResponse,$position->TITLE);
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(2,$rowResponse,$position->area->AREA_NAME);
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(3,$rowResponse,$error);
								//Aumenta la linea
								$rowResponse++;
								continue;
							}
							$position->IS_BLOCKED = "FALSE";
							//Agrega el area
							$position->_add();
							//Verifica si hubo error
							if($position->nerror > 0) {
								$errors++;
								$error = "File: $value - " . $position->POSITION_NAME . " : Sigla " . $position->TITLE . " Position error : " . $position->error;
								array_push($arrErr,$error);
								//Adiciona al response
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(0,$rowResponse,$position->POSITION_NAME);
								$objResponse->getActiveSheet()->setCellValueByColumnAndRow(1,$rowResponse,$position->TITLE);
