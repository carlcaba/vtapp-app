<?
	...
	$result = checkSession("client-partner.php",true);
						...
						<div class="card">
							...
							<div class="card-body">
								<table id="tableClient" class="table table-bordered table-striped dt-responsive nowrap">
									<thead>
										<tr>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_1"] ?></th>
											<th width="20%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_2"] ?></th>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_3"] ?></th>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_4"] ?></th>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_5"] ?></th>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_6"] ?></th>
											<th width="10%"><?= $_SESSION["PARTNER_CLIENT_TABLE_TITLE_7"] ?></th>
...
<?
	$title = $_SESSION["PARTNER_CLIENTS"];
...
?>
    ...
    <script>
	var fields = ["PARTNER_NAME", "IDENTIFICATION", "ADDRESS", "CITY", "CLIENT_NAME", "FULL_NAME", "PARTNER_CLIENT_ID", "PARTNER_ID"];
	$(document).ready(function() {
        ...
        table = $('#tableClient').DataTable({
			...
			"columns": [
				{ "data": "PARTNER_CLIENT_ID", "searchable": false, "visible": false, "responsivePriority": 8 },
				{ "data": "PARTNER_NAME", "responsivePriority": 1 },
				{ "data": "IDENTIFICATION", "responsivePriority": 3 },
				{ "data": "ADDRESS", "responsivePriority": 6 },
				{ "data": "CITY", "responsivePriority": 2 },
				{ "data": "CLIENT_NAME", "responsivePriority": 4 },
				{ "data": "FULL_NAME", "responsivePriority": 5 },
				{ "data": "PARTNER_ID", "searchable": false, "responsivePriority": 1, "sortable": false }
			],
			"autoWidth": false,
			"processing": true,
			"serverSide": true,
...
?>
		...
		$("#btnSave").on("click", function(e) {
			var form = document.getElementById('frmPartnerClient');
			var noty;
			...
			var title = $("#hfAction").val() + " <?= $_SESSION["PARTNER_CLIENT"] ?>";
			var url = $("#hfLinkAction").val();
			var $frm = $("#frmPartnerClient");
			...
			success:function(data){
				...
				if(action == "view")
					...
					$("#cbPartner").on("change", function () {
						...
						var empVal = "";
						if($("#cbEmployee").val() != "") {
							empVal = $("#cbEmployee").val();
						}
						var noty;
						$.ajax({
							url: "core/actions/_load/__loadPartnerEmployees.php",
							data: { 
								id: value,
								id_emp: empVal
							},
							...
							success:function(data){
								noty.close();
								$("#cbEmployee").find('option').remove();
								if(data.success) {
									$('#cbEmployee').append($('<option>').text("<?= $_SESSION["SELECT_OPTION"] ?>").attr('value', "").attr('selected', true));
									$.each(data.data, function(i, item) {
										$('#cbEmployee').append($('<option>').text(item.name).attr('value', item.id).attr('selected', item.selected));
									});
								}
							}
						});
