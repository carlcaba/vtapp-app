...
ALTER TABLE `TBL_AREA` DROP FOREIGN KEY `FK_AREA_CLIENT`;
ALTER TABLE `TBL_CLIENT_CARD` DROP FOREIGN KEY `FK_CLIENT_CARD_CLIENT`;
ALTER TABLE `TBL_PARTNER_CLIENT` DROP FOREIGN KEY `FK_CLIENT_PARTNER_CLIENT`;
ALTER TABLE `TBL_PAYMENT` DROP FOREIGN KEY `FK_PAYMENT_CLIENT`;
ALTER TABLE `TBL_QUOTA` DROP FOREIGN KEY `FK_QUOTA_CLIENT`;
ALTER TABLE `TBL_QUOTA_EMPLOYEE` DROP FOREIGN KEY `FK_QUOTA_EMPLOYEE_CLIENT`;
ALTER TABLE `TBL_SERVICE` DROP FOREIGN KEY `FK_SERVICE_CLIENT`;
...
ALTER TABLE `TBL_AREA` ADD CONSTRAINT `FK_AREA_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_CLIENT_CARD` ADD CONSTRAINT `FK_CLIENT_CARD_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_PARTNER_CLIENT` ADD CONSTRAINT `FK_CLIENT_PARTNER_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_PAYMENT` ADD CONSTRAINT `FK_PAYMENT_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_QUOTA` ADD CONSTRAINT `FK_QUOTA_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_QUOTA_EMPLOYEE` ADD CONSTRAINT `FK_QUOTA_EMPLOYEE_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
ALTER TABLE `TBL_SERVICE` ADD CONSTRAINT `FK_SERVICE_CLIENT` FOREIGN KEY (`CLIENT_ID`) REFERENCES `TBL_CLIENT` (`ID`);
...
SELECT
	CONCAT('ALTER TABLE `', table_name, '` ADD CONSTRAINT `', constraint_name, '` FOREIGN KEY (`', COLUMN_NAME, '`) REFERENCES `', REFERENCED_TABLE_NAME, '` (`', REFERENCED_COLUMN_NAME, '`);' ) AS agregar,
	CONCAT('ALTER TABLE `', table_name, '` DROP FOREIGN KEY `', constraint_name, '`;' ) AS borrar,
	constraint_name,
	table_name,
	COLUMN_NAME,
...
WHERE
	referenced_table_name = 'TBL_CLIENT'
	AND CONSTRAINT_SCHEMA = 'vtapp'; */
...
INSERT INTO TBL_SYSTEM_RESOURCE (RESOURCE_NAME,RESOURCE_TEXT,LANGUAGE_ID,IS_SYSTEM,IS_BLOCKED,REGISTERED_ON,REGISTERED_BY,MODIFIED_ON,MODIFIED_BY) VALUES
	 ...
	 ('TBL_AFFILIATE_SUBSCRIPTION.CREDIT_CARD_NUMBER','text,Tarjeta de crédito,Número Tarjeta de crédito,,',2,0,0,'2022-12-19 16:25:19','carlcaba',NULL,NULL),
